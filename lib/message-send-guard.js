...
const RECENT_MESSAGE_SELECT = {
  ...
  expense_comments: 'content, created_at',
};
  ...
  try {
    ...
    if (!assessment.ok) {
      return {
        ...
        retry_after_ms: assessment.retry_after_ms,
      };
    }
...
function assessMessageSpam(newContent, recentMessages, options = {}) {
  const cooldownMs = options.cooldownMs ?? SEND_COOLDOWN_MS;
  ...
  if (similarRecent.some((m) => String(m.content || '').trim() === newContent)) {
    return {
      ok: false,
      error: 'This message is the same as a recent message you sent',
      retry_after_ms: cooldownMs,
    };
  }
