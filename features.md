# Registered account features

Features available when you sign up for a personal TaskFlow account (not shared with other guest users).

- **Private account** — Your teams, tasks, and activity are tied to your own identity instead of a shared guest login.
- **Profile & avatar** — Edit your username, email, and password; choose a preset avatar or upload your own image.
- **Direct messages** — Start private one-to-one conversations with other registered users from the dashboard.
- **DM privacy controls** — Block email addresses and block or ignore users in direct messages.
- **Team chat participation** — Send, edit, and delete your own messages in team-wide chat (guests can only read).
- **Emoji reactions** — Add reactions to team chat messages, direct messages, and task comments.
- **@mentions** — Mention teammates in task comments and team chat so they are highlighted in the thread.
- **Full task editing** — Edit task title and description and delete tasks from the task panel (guests have limited editing).
- **Team invites by email** — Receive invitations at your personal email and accept or decline them on the dashboard.
- **OAuth sign-in** — Sign in with Google or GitHub in addition to email and password.
- **Manage your own messages** — Edit or delete messages you sent in team chat and direct messages.
